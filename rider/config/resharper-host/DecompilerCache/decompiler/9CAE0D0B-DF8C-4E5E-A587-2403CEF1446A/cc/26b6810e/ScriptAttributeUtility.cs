  ...
  internal class ScriptAttributeUtility
  {
    ...
    private static void BuildDrawerTypeForTypeDictionary()
    {
      ...
      foreach (System.Type type in EditorAssemblies.SubclassesOf(typeof (GUIDrawer)))
      {
        ...
        {
          CustomPropertyDrawer editor = customAttribute;
          ...
          {
            foreach (System.Type key in ((IEnumerable<System.Type>) array).Where<System.Type>((Func<System.Type, bool>) (x => x.IsSubclassOf(editor.m_Type))))
            {
              if (!ScriptAttributeUtility.s_DrawerTypeForType.ContainsKey(key) || !editor.m_Type.IsAssignableFrom(ScriptAttributeUtility.s_DrawerTypeForType[key].type))
